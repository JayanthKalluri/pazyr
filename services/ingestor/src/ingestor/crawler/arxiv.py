...
class ArxivCrawler:
    ...
    async def parse_and_filter(self, xml_data, start_date:datetime.datetime):
        ...
        for entry in root.findall("atom:entry", ns):
            try:
                ...
                published_dt = datetime.datetime.strptime(published, "%Y-%m-%dT%H:%M:%SZ").date()
                ...
                tags = [category.attrib["term"] for category in entry.findall("atom:category", ns)]
                ...
                await redis_client.push_to_stream(constants.DOWNLOADER_STREAM_NAME_REDIS, artifact_obj.model_dump_json())
            except Exception as e:
                log.error(f"Error parsing entry {entry}: {e}")
                continue

    async def fetch_artifacts(self, topics:list, start_date:datetime.datetime):
        ...
        while True:
            ...
            xml_data = await self.fetch_arxiv_papers(base_url=base_url, topics=topics, start_idx=start_idx, max_results=batch_size)
            ...
            log.debug(f"Sleeping for {config.crawler_rate_limit_seconds}s before next batch")
            await asyncio.sleep(config.crawler_rate_limit_seconds)
            
    async def close(self):
        await self.session.close()

async def crawl_arxiv(topics:list, start_date:datetime.datetime, semaphore:asyncio.Semaphore):
