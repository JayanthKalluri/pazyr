...
def start_crawlers(shutdown_event, create_task_fn):
    return [
        create_task_fn(crawler_listener(shutdown_event, create_task_fn))
    ]
...
async def process_job(msg_id:str, job: ScheduledCrawlJob, shutdown_event):
    ...
    try:
        ...
        log.error(f"Error occurred while processing job: {e}")
...
async def handle_crawler_job(job: ScheduledCrawlJob, shutdown_event):
    ...
    tasks = [
        crawl_arxiv(topics, start_date, crawler_semaphore)
    ]
...
async def pull_crawler_job() -> tuple[str, ScheduledCrawlJob] | None:
    redis_client = get_redis_client()
    messages = await redis_client.read_from_stream(stream_name=config.scheduled_crawl_job_stream_name, consumer=constants.CRAWLER_CONSUMER_NAME_REDIS, count=1, block=1000)
