...
from pazyr_core.redis_client import get_redis_client
from pazyr_core.logger import get_logger
from pazyr_core.types import JobObject, Artifact
...
def start_download_workers(shutdown_event, create_task_fn):
    return [
        create_task_fn(download_worker(i, shutdown_event)) for i in range(config.worker_count)
    ]

class DownloadWorker:
    def __init__(self, worker_id: int):
        self.worker_id = worker_id
        self.session = aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=config.download_timeout_seconds))
    ...
    async def download_artifact(self, artifact: Artifact) -> bytes | None:
        ...
        for attempt in range(config.download_retry_count):
            try:
                logger.debug(f"[{self.worker_id}] Download attempt {attempt + 1}/{config.download_retry_count} for URL: {url}")
                async with self.session.get(url) as response:
                    response.raise_for_status()
                    content = await response.read()
                    logger.debug(f"[{self.worker_id}] Successfully downloaded {len(content)} bytes from {url}")
                    return content
            except aiohttp.ClientError as e:
                wait_time = 2 ** attempt
                logger.warning(f"[{self.worker_id}] Download attempt {attempt + 1} failed for {url}: {e}. Retrying in {wait_time}s...")
                await asyncio.sleep(wait_time)
        
        logger.error(f"[{self.worker_id}] Failed to download {url} after {config.download_retry_count} attempts")
        return None
...
async def handle_failed_download(artifact: Artifact):
    ...
    await redis_client.push_to_stream(stream_name=constants.DEAD_LETTER_QUEUE_STREAM_NAME_REDIS, payload=artifact)
    ...
    try:
        while not shutdown_event.is_set():
            try:
                ...
                if not download_job:
                    logger.debug(f"[{worker_id}] No download jobs available, sleeping briefly")
                    await asyncio.sleep(0.1)
                    continue
                msg_id, artifact_obj = download_job
                
                downloaded_content = await download_worker_instance.download_artifact(artifact_obj)
                if not downloaded_content:
                    raise Exception(f"Failed to download artifact from URL: {artifact_obj.url}")
                ...
                await redis_client.push_to_stream(config.processing_stream_name, job_obj.model_dump_json())
                
                # Ack the download_job
                await redis_client.ack(constants.DOWNLOADER_STREAM_NAME_REDIS, msg_id)
                
                logger.info(f"[{worker_id}] Successfully processed download job {job_obj.id} for artifact {artifact_obj.id}")
                    
            except Exception as e:
                logger.warning(f"[{worker_id}] Error in download worker loop: {e}", exc_info=True)
